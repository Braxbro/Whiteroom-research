#!/bin/bash
# Stage 5b: Unfrozen curriculum fine-tuning on top of Stage 5 checkpoints.
# All seeds run in parallel; each gets N_WORKERS data workers.
#
# Usage: bash train_stage5b_all.sh [STAGE5_DIR] [OUTDIR]
set -e
STAGE5_DIR=${1:-_agent/cache/runs/stage5}
OUTDIR=${2:-_agent/cache/runs/stage5b}
N_WORKERS=3   # per-seed; 5 seeds * 3 = 15 total CPU workers
PYTHON=/home/babrook/pytorch_env/bin/python
mkdir -p "$OUTDIR"
cd /home/babrook/Documents/research

LOG="$OUTDIR/run_log.txt"
echo "Stage 5b parallel launch — $(date)" | tee "$LOG"

pids=()
for seed in 1 2 3 4 5; do
    PRIMARY="$STAGE5_DIR/stage5-seed${seed}/checkpoint_final.pt"
    SEEDDIR="$OUTDIR/stage5b-seed${seed}"
...
$PYTHON _agent/scripts/eval/eval_multiseed.py \
    --rundir "$OUTDIR" \
    --subdir-prefix stage5b \
    --seeds 1,2,3,4,5 \
    2>&1 | tee -a "$LOG"

REPORT="$OUTDIR/summary.txt"
echo "Stage 5b Summary — $(date)" > "$REPORT"
echo "" >> "$REPORT"
$PYTHON -c "
