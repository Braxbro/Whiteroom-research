#!/bin/bash
# Stage 7a: bidirectional BIND, standard (bidirectional) encoder
# Trains with 50/50 A-BIND-B / B-BIND-A ordering → BIND becomes logical operator, not positional cue
# 5 seeds in parallel via shared data server
set -e
OUTDIR=${1:-_agent/cache/runs/stage7/7a-bidir-bind}
SEEDS=${2:-1,2,3,4,5}
mkdir -p "$OUTDIR"
...
echo "=== Stage 7a: bidir-bind ===" | tee -a "$LOG"
...
echo "" | tee -a "$LOG"
echo "=== 7a training done ===" | tee -a "$LOG"
