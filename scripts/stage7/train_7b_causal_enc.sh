#!/bin/bash
# Stage 7b: causal (unidirectional) encoder, standard directional BIND
# A tokens cannot attend to B; A-freeze should be free by construction
# 5 seeds in parallel via shared data server
set -e
OUTDIR=${1:-_agent/cache/runs/stage7/7b-causal-enc}
SEEDS=${2:-1,2,3,4,5}
mkdir -p "$OUTDIR"
...
echo "=== Stage 7b: causal-encoder ===" | tee -a "$LOG"
...
echo "" | tee -a "$LOG"
echo "=== 7b training done ===" | tee -a "$LOG"
