#!/bin/bash
# Stage 8d: Fine-tune linear projection + decoder (unfrozen)
...
echo "=== Stage 8d: Linear Projection Fine-Tuning (Unfrozen Decoder) ==="
echo "Starting: $(date)"
...
for seed in 1 2 3 4 5; do
  ...
  STAGE8_CKPT="$STAGE8_DIR/7d-seed${seed}_stage5-seed${seed}/checkpoint_translation.pt"
  PAIR_DIR="$RUN_DIR/8d-linear-unfreeze-seed${seed}"
  ...
  mkdir -p "$PAIR_DIR"
  echo "Launching 8d seed $seed (linear projection, unfrozen decoder) in background..."
...
echo "================================"
echo "All 8d seeds launched in parallel!"
echo "Results in: $RUN_DIR/8d-linear-unfreeze-seed*"
echo "================================"
echo "Waiting for all training to complete..."
wait
echo "All 8d training complete!"
echo "Finished: $(date)"
