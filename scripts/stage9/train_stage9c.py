...
def train_stage9c(
    ...
    def set_freezing(phase_str: str):
        """Apply freezing based on current phase (CORRECTED: decoder-biased)."""
        if phase_str in ("1A", "2A"):
            # Encoder frozen, bridge/decoder free (decoder pulls bridge toward its needs)
            model.encoder.requires_grad_(False)
            model.adaptation.requires_grad_(True)
            model.decoder.requires_grad_(True)
            print(f">>> Phase {phase_str}: Encoder FROZEN, bridge/decoder FREE (decoder-biased)")
        elif phase_str in ("1B", "2B"):
            # Bridge frozen, encoder/decoder free (encoder adapts to decoder-optimized bridge)
            model.adaptation.requires_grad_(False)
            model.encoder.requires_grad_(True)
            model.decoder.requires_grad_(True)
            print(f">>> Phase {phase_str}: Bridge FROZEN, encoder/decoder FREE (encoder adapts)")
