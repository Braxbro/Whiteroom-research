#!/bin/bash
# Stage 7b (50K model): causal encoder only
# Test if structural isolation generalizes to smaller model.
# Model dims: d_model=32, nhead=2, enc_layers=2, dec_layers=2, ffn_dim=128
set -e
OUTDIR=${1:-_agent/cache/runs/stage7_50k/50k-7b-causal-enc}
SEEDS=${2:-1,2,3,4,5}
mkdir -p "$OUTDIR"
...
echo "=== Stage 7b (50K model): causal-encoder ===" | tee -a "$LOG"
...
echo "" | tee -a "$LOG"
echo "=== 7b (50K) training done ===" | tee -a "$LOG"
