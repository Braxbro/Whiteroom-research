#!/bin/bash
# Stage 7c (50K model): causal encoder + bidirectional BIND
# Test if constraint conflict still occurs at smaller scale.
# Model dims: d_model=32, nhead=2, enc_layers=2, dec_layers=2, ffn_dim=128
set -e
OUTDIR=${1:-_agent/cache/runs/stage7_50k/50k-7c-causal-bidir}
SEEDS=${2:-1,2,3,4,5}
mkdir -p "$OUTDIR"
...
echo "=== Stage 7c (50K model): causal-encoder + bidir-bind ===" | tee -a "$LOG"
...
echo "" | tee -a "$LOG"
echo "=== 7c (50K) training done ===" | tee -a "$LOG"
